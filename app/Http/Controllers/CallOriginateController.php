...
class CallOriginateController extends Controller
{
    ...
    {
        $apiKey = 'qRb2e8veTMAPRJEg@Q33xsE_MPRI:B-4_ELgXw*BNVVfS';
        $asteriskUrl = 'http://114.143.197.210:8088/ari/events/user/originate-call?application='.$stasisApp;
        $request = ['variables' => [
    ...
    private function triggerCurl($request, $asteriskUrl, $apiKey, $timeout = 10)
    {
        $ch = curl_init();
    ...
    private function logger($data)
    {
        ...
        $filename = storage_path().'/app/originateCallLog/originateCallLog-'.date('Ymd').'.log';
    ...
    public static function requestLog($loggerData)
    {
        ...
        $path = storage_path().'/app/originateCallLog/originateCallRequestLog.csv';
        $file = fopen($path, 'a');
    ...
    public static function responseLog($loggerData) 
    {
        ...
        $path = storage_path().'/app/originateCallLog/originateCallResponseLog.csv';
        $file = fopen($path, 'a');
        fputcsv($file, $csv_data);
        fclose($file);
    }
